...
function MobileMenu({ open, onClose }) {
  ...
  if (!open) return null;
  return (
    <div className="absolute top-12 left-0 right-0 bg-white/95 backdrop-blur-md border-b border-plt-border shadow-lg z-[1999] flex flex-col py-2">
...
function AppNav() {
  ...
  return (
    <nav ref={navRef} className="bg-white/90 backdrop-blur-md border-b border-plt-border px-4 sm:px-6 flex items-center h-12 flex-shrink-0 z-[2000] relative">
      {/* Logo */}
      <div className="flex items-center gap-2 mr-3 md:mr-10 flex-shrink-0">
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        <button
          className="md:hidden flex flex-col justify-center items-center w-8 h-8 gap-1.5 rounded hover:bg-plt-hover/50 transition-colors"
          onClick={() => setMenuOpen(o => !o)}
          aria-label="Toggle menu"
        >
          <span className={`block w-5 h-0.5 bg-plt-secondary transition-transform origin-center ${menuOpen ? "rotate-45 translate-y-2" : ""}`} />
          <span className={`block w-5 h-0.5 bg-plt-secondary transition-opacity ${menuOpen ? "opacity-0" : ""}`} />
          <span className={`block w-5 h-0.5 bg-plt-secondary transition-transform origin-center ${menuOpen ? "-rotate-45 -translate-y-2" : ""}`} />
        </button>
      </div>
